...
% assert(rem(ns,nworkers)==0,"nsamples must be divisible by nworkers\n");
% wsamples = ns/nworkers;
% 
...
% 
% workerlocs = cell(nworkers,1);
% for i = 1:nworkers
%     workerlocs{i} = [1:wsamples] + (i-1)*wsamples;
% end

spmd
    [wxdata,wydata] = datagen(data,wsamples,ws,nss,ovl,exl);
end

for i = 1:nworkers
    xdata(workerlocs{i}) = wxdata{i};
    ydata(workerlocs{i}) = wydata{i};
end
