classdef generator < DeepNetwork
    ...
    methods
        function obj = generator(layersizes,WindowSize,Overlap,nSubsamples)
             ...
             obj.info.training      = true;             % boolean denoting if network is training
        ...
        function [dly,varargout] = predict(obj,x,varargin)
            ...
            function dlx = subsample_data(obj,x)
                ...
                if isempty(Set)
                    Set = gpudl(zeros([obj.info.WindowSize size(x,2) obj.info.nSubsamples],obj.DataType),'');     % Initialize set as zeros
                end

                locs = [1:windowsize];                                          % Initialize locs 
                ...
                dlx = permute(Set,[1 3 2]);    % Permute set so its dimensions are TCB           
            end
        end
        ...
        function [dly,FineTuneCoeff,ScaleFactor] = scalinglayer(obj,x)
            ...
            FineTuneCoeff = FineTuneScaling(dlx);       % obtain fine tune coefficient
        ...
        function [layer,info] = init_waveletlayer(obj,layersizes)
