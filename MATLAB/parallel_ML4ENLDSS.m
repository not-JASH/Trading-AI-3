...
ns = 5e3;
assert(rem(ns,nw-1)==0,"nsamples must be divisible by nworkers-1\n");
wsamples = ns/(nw-1);
...
spmd 
    if spmdIndex == 1
        ...
        locs = [1:wsamples];
        for i = 2:nw
            xdata(locs) = spmdReceive(i,1);
            ydata(locs) = spmdReceive(i,2);
            locs = locs+wsamples;
        end
    else
        gpuDevice([]);
        [xdata,ydata] = datagen(data,wsamples,ws,nss,ovl,exl);
...
%             fprintf("finished training for epoch %d, time elapsed: %.2f\n",epoch,toc);
            ...
            spmdy = gatext(gen_predict(gen,validx,validxh,ws,nss,ovl));
            spmdY = squeeze(gatext(validy.y));
            spmd_avg_e = mean(abs(spmdY-spmdy),'all');
            
%             fprintf("finished training and evaluation for epoch %d, time elapsed: %.2f\n",epoch,toc);

            % receive new x and ydata 
            spmdBarrier;
            locs = [1:wsamples];
            for i = 2:nw
                xdata(locs) = spmdReceive(i,1);
            ...
            [xdata,ydata] = datagen(data,wsamples,ws,nss,ovl,exl);
%             fprintf("finished generating samples, time elapsed: %.2f\n",toc);
            spmdBarrier;
...
function [xdata,ydata] = datagen(data,ns,ws,nss,ovl,exl)
    ...
    function y = scale_detrend_reweight(x,subsampleflag)  
        ...
        y = reweight_detrend(y,subsampleflag,ws,ovl,nss);    % reweight and detrend dat
...
function [y,x_rwdt,y_pl] = gen_predict(gw,x,xh,ws,nss,ovl) % retrended y, reweighted detrended x, prediction layer y
    ...
    dly_set                         = gpuwaveletlayer(x_rwdt);                                     % Wavelet transform dataset.
...
function [y] = disc_predict(dw,x)
    ...
    y = gpuwaveletlayer(permute(x,[1 3 2]));       % wavelet transform input data
...
function y = estimate_encodinglayer(lw,x)
    ...
    y = gpuwaveletlayer(dlx);        % Wavelet transform the batch of estimates
